...
SimpleAlarmClock::SimpleAlarmClock(byte _rtc_Address, byte _eeprom_Address, bool alarmIntEnabled)
{
    ...
    dateTimeMock.Year = 20;     // calendar yyy, note 2000 must be addded
...
static uint32_t cntL2 = 0;
    ...
    if (tick)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            dateTimeMock.Hour = (dateTimeMock.Hour < 24) ? ++dateTimeMock.Hour : 0;
            break;
        case 4:
            dateTimeMock.Minute = (dateTimeMock.Minute < 60) ? ++dateTimeMock.Minute : 0;
            break;
        default:
            break;
        }
...
AlarmTime SimpleAlarmClock::readRtcAlarm(byte alarmSelected){
    ...
    alarm_i.EnabledDows = readAlarmEnabledDows(alarmSelected);
...
byte SimpleAlarmClock::readAlarmEnabledDows(byte alarmSelected) {
    // Read DOWs for alarms - I assigned it 0x09 and 0x10 addresses.
    // alarm1 @ 0x09
    // alarm2 @ 0x10
    byte reg = (alarmSelected == 1)? 0x09 : 0x10;
    return readMem(reg);
}

byte SimpleAlarmClock::nextAlarmDay(byte _AlarmMode, byte _ClockMode, byte EnabledDows, byte _Hour, byte _Minute){
    /** ******************************************************************
...
byte SimpleAlarmClock::setRtcAlarm(const AlarmTime &alarm_i, byte alarmSelected){
     ...
     * --------------------------------------------------------------------
     * |  0  |  0  |  0  |  0  |  0  | Alarm date, hrs, min,(& sec) match - date: 1-31
     * --------------------------------------------------------------------
     * |  1  |  0  |  0  |  0  |  0  | Alarm day, hrs, min,(& sec) match - day: 1-7
     * --------------------------------------------------------------------
     ****************************************************************** */
    /* AlarmTime structure:
     byte Second;       // 0-59 = 6 bits 0=for alarm2
    ...
    ******************************************************************* */
    ...
    _Dow = nextAlarmDay(alarm_i.AlarmMode, alarm_i.ClockMode, alarm_i.EnabledDows, alarm_i.Hour, alarm_i.Minute);
...
/*    // If Daily make A1M4/A2M4 = 1
    ...
    // Now Daily can have defined specific days it is enabled on
    //A1M4/A2M4 = 0, DY/DT = 1
    _byteValue[2] |= (1<<6);
...
void SimpleAlarmClock::setAlarmEnabledDows(byte enabledDows, byte alarmSelected) {
    // Set EnabledDows for alarms - I assigned it 0x09 and 0x10 addresses.
    // alarm1 @ 0x09
    // alarm2 @ 0x10
    byte reg = (alarmSelected == 1)? 0x09 : 0x10;
    writeMem(reg, enabledDows);
}
